...
本文件已按当前前端修订：资料已并入“我的”选项卡；红娘工作台已有可见“和会员聊天”区域；静态页通过 app.js?v=<Git短提交号> 自动破缓存。
...
- 包含左侧导航栏，可进入红娘/管理员工作台
- 客户业务不再内嵌模拟器，统一跳转真实 uniapp H5
...
- 由 uniapp 构建 H5 和微信小程序
- 功能：登录/注册、发现、个人资料、服务订阅、牵线请求、聊天
...
### matchmaker.html — 红娘工作台（8097/21314）
...
| 键名 | 内容 |
|------|------|
| `matchmaker-dating-demo-v1` | 完整 state JSON |
| `matchmaker-dating-demo-v1:session` | 会话信息 `{currentUserId, selectedMatchmakerId, adminLoggedIn, token, role}` |
...
- 找到键 `matchmaker-dating-demo-v1`（完整 state）
- 找到键 `matchmaker-dating-demo-v1:session`（会话信息）

**控制台操作**：

```javascript
// 读取完整 state
const raw = localStorage.getItem('matchmaker-dating-demo-v1');
const data = JSON.parse(raw);
console.log(data);

// 读取会话
const sessionRaw = localStorage.getItem('matchmaker-dating-demo-v1:session');
console.log(JSON.parse(sessionRaw));

// 手动清空（重置数据）
localStorage.removeItem('matchmaker-dating-demo-v1');
localStorage.removeItem('matchmaker-dating-demo-v1:session');
location.reload();
```
...
localStorage.setItem('matchmaker-dating-demo-v1', JSON.stringify(data));
location.reload();
```
