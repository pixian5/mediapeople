...
```
matchmaker/
├── index.html              综合预览端（8095/21314），可在页面内切换三端视角
├── uniapp/                 真实客户 H5/微信小程序（8096/21314）
├── matchmaker.html         红娘工作台端（8097/21314）
├── admin.html              管理后台端（8098/21314，使用 __ASSET_VERSION__ 占位符）
...
├── compose.ssl.yml         Docker Compose：HTTPS 前端容器（21314）
├── .env.example            环境变量示例
├── .gitignore              Git 忽略规则
...
- `ensureUserDefaults()` — 补全客户默认字段（servicePlans、matchmakerIds）
...
├── auto-deploy.sh          服务器自动部署脚本（由 GitHub Actions SSH 或手动触发）
...
| 列名 | 小写，下划线分隔 | `real_name_verified` |
| 主键 | `id` | `id` |
...
graph TD
    ...
    Git --> Actions[GitHub Actions SSH]
    Actions --> AutoDeploy[auto-deploy.sh]
    AutoDeploy --> DockerCompose[docker compose up -d --build]
    
    Manual[手动 SSH] -.手动触发.-> AutoDeploy
```
