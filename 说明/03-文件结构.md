...
- `PATCH /api/matchmaker/requests/:id/approve-member-chat` — 开启会员互聊
...
└── mediapeople-webhook.service  systemd 服务配置文件
```
...
检查是否正在部署
  ...
  └─ 空闲 → 开始部署
  ↓
执行 bash /opt/mediapeople/deploy/auto-deploy.sh
  ├─ 成功 → 记录日志
  └─ 失败 → 记录错误日志
```
...
### mediapeople-webhook.service

systemd 服务配置文件，用于将 Webhook 服务注册为系统服务：

```ini
[Unit]
Description=MediaPeople GitHub Webhook
After=network.target

[Service]
ExecStart=/usr/bin/node /opt/mediapeople/webhook/index.js
Restart=always
Environment=WEBHOOK_SECRET=your_secret
Environment=DEPLOY_SCRIPT=/opt/mediapeople/deploy/auto-deploy.sh
...
- 由 Docker volume 挂载
- 服务器路径：`/opt/mediapeople/data/postgres`
- 包含所有数据库文件
...
| 类型 | 规则 | 示例 |
|------|------|------|
| HTML入口 | 小写，短横线分隔 | `index.html`, `mini.html`, `matchmaker.html` |
...
```mermaid
graph TD
    HTML[index.html / mini.html / matchmaker.html / admin.html] --> CSS[styles.css]
    HTML --> JS[app.js]
...
```mermaid
graph TD
    Local[本地 deploy.sh] --> Git[Git push to GitHub]
