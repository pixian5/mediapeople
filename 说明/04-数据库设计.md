...
本文件已按当前业务数据修订：用户支持 vipMatchmakerIds、delegatedMatchmakerIds、profileByMatchmaker；牵线请求包含男方/女方联系状态和 memberChatEnabled；聊天线程包含 member_matchmaker 与 member_member。
...
数据库使用 **PostgreSQL 16**（Alpine 镜像），通过 Docker 容器运行，数据持久化到宿主机 `/opt/mediapeople/data/postgres`。
...
- 删除红娘时，关联客户的 `referral_matchmaker_id` 置为 NULL
...
- `-Fc`：自定义格式，压缩存储
- 备份文件保存在 `/opt/mediapeople/backup/postgres/`
...
erDiagram
    ...
    users {
        ...
        TEXT email
        BOOLEAN vip
        TEXT referral_matchmaker_id FK
        TEXT password_hash
        TEXT account_status
        TIMESTAMPTZ registered_at
    ...
    matchmakers ||--o{ users : "referral_matchmaker_id"
...
-- users 表外键索引
CREATE INDEX idx_users_referral_matchmaker_id ON users(referral_matchmaker_id);
...
-- users.raw：用于 vipMatchmakerIds、delegatedMatchmakerIds 等数组查询
...
| Q4 | 获取红娘的客户列表 | `users` | 高 | 中（按 referral_matchmaker_id 过滤） | 外键索引已建议 |
...
| Q15 | JSON 字段内查询（如 vipMatchmakerIds） | `users` | 中 | 中 | GIN 索引 raw 字段 |
...
| `name` | TEXT | NOT NULL | - | 机构名称 | `优联婚恋传媒` |
...
| `referral_matchmaker_id` | TEXT | REFERENCES matchmakers(id) ON DELETE SET NULL | NULL | 推荐红娘ID | `m1m2n3o4cd` |
...
**`raw` 字段额外字段**：
- `vipMatchmakerIds`: VIP红娘ID列表（数组）
- `delegatedMatchmakerIds`: 委托红娘ID列表（数组）
...
```bash
#!/bin/bash
# /opt/mediapeople/scripts/full-backup.sh

BACKUP_DIR="/opt/mediapeople/backup/postgres"
DATE=$(date +%F_%H%M%S)
...
# 验证备份文件
docker exec mediapeople-postgres pg_restore -l "/backup/full-backup-$DATE.dump" > /dev/null 2>&1
if [ $? -eq 0 ]; then
...
# 添加以下内容
# 每日全量备份（凌晨2:00）
0 2 * * * /opt/mediapeople/scripts/full-backup.sh >> /opt/mediapeople/backup/postgres/backup.log 2>&1

# 每周全量备份（周日凌晨3:00，保留90天）
0 3 * * 0 /opt/mediapeople/scripts/full-backup.sh weekly >> /opt/mediapeople/backup/postgres/backup.log 2>&1
```
...
# 2. 恢复基础备份
docker exec mediapeople-postgres pg_basebackup ... 
...
```bash
#!/bin/bash
# /opt/mediapeople/scripts/restore-test.sh

echo "=== 恢复演练开始 ==="
date

# 1. 准备测试环境
TEST_CONTAINER="mediapeople-postgres-test"
TEST_PORT=5433
BACKUP_FILE=$(ls -t /opt/mediapeople/backup/postgres/full-backup-*.dump | head -1)

echo "使用备份文件: $BACKUP_FILE"

# 2. 启动临时 PostgreSQL 容器
docker run -d --name "$TEST_CONTAINER" \
  -e POSTGRES_USER=mediapeople \
  -e POSTGRES_PASSWORD=mediapeople \
  -e POSTGRES_DB=mediapeople_test \
  -p "$TEST_PORT:5432" \
  -v /opt/mediapeople/backup/postgres:/backup \
  postgres:16-alpine
...
if [ $? -eq 0 ]; then
  ...
  # 5. 数据验证
  docker exec "$TEST_CONTAINER" psql -U mediapeople -d mediapeople_test -c "
    SELECT 
