...
本文件已按当前部署架构修订：线上以 21314 HTTPS 入口为主；静态 HTML 由 scripts/render-static.mjs 在服务器生成自动 Git 版本号；服务器通过 deploy/auto-deploy.sh 自动拉取 Git 并部署。
...
│  │ 8095/21314│  │ 8096/21314│  │ 8097/21314│  │8098/ │ │
│  │          │  │          │  │          │  │21314  │ │
│  └────┬─────┘  └────┬─────┘  └────┬─────┘  └──┬───┘ │
│       └──────────────┴────────────┴────────────┘     │
│                      ▼ HTTP /api/*                    │
...
后端验证密码 → 签发 HMAC Token（7 天有效期）
    ↓
前端存储 token → localStorage: matchmaker-dating-demo-v1:session
    ↓
...
|------|-----------|------------|-----------|
| web | 8095 | 21314 | index.html |
| web-mini | 8096 | 21314 | uniapp/dist/build/h5 |
| web-matchmaker | 8097 | 21314 | matchmaker.html |
| web-admin | 8098 | 21314 | admin.html |
...
sequenceDiagram
    ...
    Express->>Express: 业务校验<br/>1. 发起方必须是VIP<br/>2. 发起方存在该红娘的有效 servicePlans[].matchmakerId<br/>3. 目标方必须绑定该红娘<br/>4. 检查是否已有未完成请求
